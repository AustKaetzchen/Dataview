 ...
 */
export const DataLayersTab: React.FC<DataLayersTabProps> = function (arg0_props) {
  ...
  active_layer = useMemo(() => {
    ...
    for (let i = 0; i < keys.length; i++) {
      let l = layers[keys[i]]
      if (l.sub_layers) {
        let found = l.sub_layers.find((arg0_sub) => arg0_sub.id === active_layer_id)
        if (found)
          return found
      }
    }
            ...
            >
              ...
              {arg0_layer.sub_layers && arg0_layer.sub_layers.length > 0 && (
                <div className="px-2.5 pb-2 pt-1 border-t border-border/40 bg-card/30 flex items-center gap-1 flex-wrap">
                  ...
                  {arg0_layer.sub_layers.map((arg0_sub) => {
