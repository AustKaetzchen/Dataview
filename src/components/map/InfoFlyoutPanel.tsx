  ...
  return (
    <div className="absolute bottom-11 left-0 z-30 w-96 sm:w-[420px] max-h-[calc(100vh-160px)] flex flex-col bg-card/98 backdrop-blur-md border border-border shadow-2xl p-[var(--padding)] text-[var(--body-font-size)] font-sans select-none animate-in fade-in-0 zoom-in-95 duration-100">
      ...
      <Tabs
        ...
        className="flex flex-col flex-1 overflow-hidden mt-[var(--cell-padding)]"
      >
            ...
            >
              {tab.type === 'controls' ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] px-1.5 py-0.2 bg-primary/20 text-primary border border-primary/40 font-semibold">
                      ...
                      {activeModes.length === 0 ? (
                        ...
                        activeModes.map((m) => {
                          ...
                          return (
                            <div
                              key={m.id}
                              className="flex items-start justify-between gap-2 text-[var(--body-font-size)] leading-snug"
                            >
                  ...
                  <div className="flex items-center justify-between p-2 bg-background/40 border border-border text-[11px]">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="space-y-1 bg-background/60 p-2 border border-border">
                      {(tab.shortcuts || []).map((sc, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between py-1 border-b border-border/40 last:border-0 text-[var(--body-font-size)]"
                        >
                          <kbd className="px-1.5 py-0.5 bg-muted text-foreground border border-border text-[11px] font-mono font-bold shadow-sm">
                            {sc.key}
                          </kbd>
                          <span className="text-muted-foreground text-right text-[11px] ml-2">
                            {sc.action}
                          </span>
                        </div>
                      ))}
                ...
                <div className="space-y-3 text-[var(--body-font-size)]">
                  {/* Title & Badge */}
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-foreground text-[var(--header-font-size)]">
                      {tab.title || tab.label}
                    </span>
                    {tab.badge && (
                      <span className="text-[11px] px-2 py-0.5 bg-primary/20 text-primary border border-primary/40 font-bold tracking-wide">
                        {tab.badge}
                      </span>
                    )}
                  </div>
